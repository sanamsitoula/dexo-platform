...
export default function ArAgingPage() {
  ...
  useEffect(() => {
    ...
    tenantFinanceReportsApi.getArAging(subdomain, asOfDate).then((r) => {
  ...
  return (
    <div>
      <ReportHeader
        title="AR Aging"
        subtitle="Receivables from members bucketed by days past issue"
        startDate={asOfDate}
        endDate={asOfDate}
        onExport={() => data?.rows && downloadCsv(`ar-aging_${asOfDate}.csv`, data.rows.map((r: any, i: number) => ({
          invoiceNumber: r.invoiceNumber, customerName: r.customerName, invoiceDate: formatDate(r.invoiceDate),
        ...
        <>
          ...
          <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded">
            <div className="text-sm text-amber-900">
              Total outstanding: <strong className="font-mono">{formatNumber(data.totalOutstanding)}</strong> across <strong>{data.count}</strong> invoices
            </div>
          </div>
