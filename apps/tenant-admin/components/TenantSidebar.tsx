...
import { useTenantAuth } from '@/components/TenantAuthContext'
import { getDomainMenus, getDomainTheme } from '@/lib/domain-config'
...
export default function TenantSidebar() {
  ...
  const menus = getDomainMenus(domainCode)
  const theme = getDomainTheme(domainCode)
  ...
  const icons: Record<string, JSX.Element> = {
    ...
    scissors: (
      <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 3.75H6A2.25 2.25 0 003.75 6v1.5M16.5 3.75H18A2.25 2.25 0 0120.25 6v1.5m0 9V18A2.25 2.25 0 0118 20.25h-1.5m-9 0H6A2.25 2.25 0 013.75 18v-1.5M15 6.75a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
    clipboard: (
      <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
          ...
          {menus.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <span>{icons[item.icon] || icons.document}</span>
