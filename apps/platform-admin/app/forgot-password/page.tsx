      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
        {sent ? (
          <div className="text-center">
            ...
            <Link href="/login" className="inline-block mt-6 text-sm font-medium text-indigo-600 hover:text-indigo-500">← Back to sign in</Link>
          </div>
        ) : (
          <form onSubmit={submit}>
            ...
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@yourcompany.com"
            ...
            <button type="submit" disabled={busy || !email}
              className="mt-5 w-full rounded-lg py-2.5 text-white font-semibold bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50">
              {busy ? 'Sending…' : 'Send reset link'}
            </button>
            <Link href="/login" className="block text-center mt-4 text-sm font-medium text-indigo-600 hover:text-indigo-500">← Back to sign in</Link>
          </form>
        )}
      </div>
    </div>
